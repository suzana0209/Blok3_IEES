...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        private Dictionary<DMSType, ResourceDescription> CreateResourcesToInsert()
        {
            ...
            SetRegulationScheduleReferences(updates);
            SetTapScheduleReferences(updates);
            SetRegularTimePointReferences(updates);
        ...
        private void SetRegularTimePointReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.REGULARTIMEPOINT].Properties.Count; i++)
            {
                if (updates[DMSType.REGULARTIMEPOINT].Properties[i].Id == ModelCode.REGULARTIMEPOINT_REGULARINTERVALSCHEDULE)
                {
                    updates[DMSType.REGULARTIMEPOINT].Properties[i].SetValue(updates[DMSType.TAPSCHEDULE].Id);
                    //updates[DMSType.REGULARTIMEPOINT].Properties[i].SetValue(updates[DMSType.REGULATIONSCHEDULE].Id);
                }
            }
        }

        private void SetRegulationScheduleReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.REGULATIONSCHEDULE].Properties.Count; i++)
            {
                if (updates[DMSType.REGULATIONSCHEDULE].Properties[i].Id == ModelCode.SEASONDAYTYPESCHEDULE_DAYTYPE)
                {
                    updates[DMSType.REGULATIONSCHEDULE].Properties[i].SetValue(updates[DMSType.DAYTYPE].Id);
                }
                if (updates[DMSType.REGULATIONSCHEDULE].Properties[i].Id == ModelCode.REGULATIONSCHEDULE_REGULATINGCONTROL)
                {
                    updates[DMSType.REGULATIONSCHEDULE].Properties[i].SetValue(updates[DMSType.REGULATINGCONTROL].Id);
                }
            }
        }

        private void SetTapScheduleReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.TAPSCHEDULE].Properties.Count; i++)
            {
                if (updates[DMSType.TAPSCHEDULE].Properties[i].Id == ModelCode.TAPSCHEDULE_TAPCHANGER)
                {
                    updates[DMSType.TAPSCHEDULE].Properties[i].SetValue(updates[DMSType.TAPCHANGER].Id);
                }
                if (updates[DMSType.TAPSCHEDULE].Properties[i].Id == ModelCode.SEASONDAYTYPESCHEDULE_DAYTYPE)
                {
                    updates[DMSType.TAPSCHEDULE].Properties[i].SetValue(updates[DMSType.DAYTYPE].Id);
                }
            }
        }
